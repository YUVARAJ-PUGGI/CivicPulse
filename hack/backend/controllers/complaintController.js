...
class ComplaintController {
  ...
  static async resolveComplaint(req, res) {
    ...
    try {
      ...
      console.log('Fetching complaint details for notification...');
      const [complaintDetailsRows] = await connection.execute(
        'SELECT user_id, title, category FROM complaints WHERE id = ?',
        [id]
      );
      
      if (complaintDetailsRows.length > 0) {
        const complaintDetails = complaintDetailsRows[0];
        console.log('Creating resolution notification...');
        
        // Create notification for citizen
        const notificationQuery = `
          INSERT INTO notifications 
          (user_id, complaint_id, type, message, status, is_read, created_at)
          VALUES (?, ?, ?, ?, ?, ?, NOW())
        `;
        const notificationMessage = `Your complaint "${complaintDetails.title}" has been resolved!`;
        const notificationParams = [
          complaintDetails.user_id,
          id,
          'resolution',
          notificationMessage,
          'unread',
        ...
        try {
          await connection.execute(notificationQuery, notificationParams);
          console.log('✓ Resolution notification created for user:', complaintDetails.user_id);
        } catch (notifError) {
          console.error('⚠️ Warning: Could not create notification:', notifError.message);
